...
export default function WordPair({dataDict, setCorrectWord, correctWord, setIsFinished}: WordPairProps) {
    ...
    return (
        <Card className="w-full max-w-md mx-auto">
            <CardHeader>
                <CardTitle>Word Pair Practice</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                    ...
                    <div key={lang}>
                        ...
                        {index !== 0 && displayResponse && (
                            <Input
                                ...
                                value={dataDict[languages[1]][currentIndex].toLowerCase() || ''}
                                className={currentWords[1].toLowerCase() === dataDict[languages[1]][currentIndex].toLowerCase() ? "mt-1 bg-green-500 text-white" :  "mt-1 bg-red-500 text-white"}
                                readOnly={true}
                            />
                        )}
                    </div>
                ...
                >
                    {displayResponse ? "Continue" : "Check Answer"}
                </button>
            </CardContent>
        </Card>
    )
